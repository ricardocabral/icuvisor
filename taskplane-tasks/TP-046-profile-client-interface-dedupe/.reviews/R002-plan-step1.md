...
- Record the exactness result in `STATUS.md`: the two declarations have the same single method, `GetAthleteProfile(context.Context) (intervals.AthleteWithSportSettings, error)`, and differ only in doc-comment wording (`tools` vs `resources`).
