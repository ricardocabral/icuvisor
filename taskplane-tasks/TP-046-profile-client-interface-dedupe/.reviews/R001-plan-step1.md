...
- Record the current exactness result: the method set/signature is identical, while the doc comments differ only by consumer wording (`tools` vs `resources`). The shared doc comment should preserve the fact that both tools and resources consume it.
