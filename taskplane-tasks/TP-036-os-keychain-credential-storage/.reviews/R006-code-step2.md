...
- `go mod tidy` would reclassify `github.com/zalando/go-keyring` and `github.com/godbus/dbus/v5` as direct dependencies and add checksum entries for the new module graph's test dependencies. This is not a Step 2 blocker, but it is worth tidying before merge if the project/CI enforces module-file cleanliness.
