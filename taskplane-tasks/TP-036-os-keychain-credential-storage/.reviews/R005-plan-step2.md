...
- **Context behavior:** the plan correctly acknowledges that `go-keyring` is not context-aware and commits to checking `ctx.Err()` before and after calls, without using cancellation goroutines that could hide a later `Set` side effect.
