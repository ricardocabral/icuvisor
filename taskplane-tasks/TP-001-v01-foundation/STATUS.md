...
- `.env` scope: parse a local `.env` read-only with stdlib code, accept only recognized `INTERVALS_ICU_*` and `ICUVISOR_*` keys, ignore notes/unknowns, and never print loaded values.
