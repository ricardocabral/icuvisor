...
3. **Define the first-seen state model, not just the fields.**
   ...
   The per-process fallback caveat is acceptable because the SDK session handle is not available at the response shaper boundary, but it needs a code comment/docstring explaining that this only supports simulated in-process divergence and future session plumbing, not real cross-process restart detection.
