...
- Re-ran the SDK dependency graph check in a temporary module with `the MCP Go SDK/mcp@v1.3.1`; `go list -m all` reports the same newly introduced modules now recorded in `STATUS.md`, including `cloud.google.com/go/compute/metadata v0.3.0`.
