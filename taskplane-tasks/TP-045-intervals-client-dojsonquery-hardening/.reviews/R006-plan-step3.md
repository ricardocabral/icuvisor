...
1. **Add an exported-method doc comment.**  
   `RetryConfig.WithDefaults` will be exported, and this repo has `revive`'s exported-comment rule enabled. Add a comment starting with `WithDefaults`, e.g. `// WithDefaults returns c with retry defaults applied...`.
