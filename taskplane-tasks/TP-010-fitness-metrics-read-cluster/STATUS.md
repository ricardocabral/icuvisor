...
| RPE, feel, session-RPE                                                                                              | Activity list/detail subjective fields; wellness/daily fields for same-date subjective scales; activity messages/comments only as evidence of absence, not as structured metric sources                                                                             |
...
- Fixtures must contain only fields needed to prove availability/absence decisions; scrub API keys, raw athlete IDs, exact activity IDs when possible, notes/comments, GPS/location traces, device serials, and personally identifying free text.
