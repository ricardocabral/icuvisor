...
- Step 4 plan: change the checked-in athlete profile fixture to a RUN setting with `threshold_pace: 3.5714285`, display-only `MINS_KM`, `pace_load_type: RUN`, and percentage zones `[77.5,90,100]`. Add a table-driven semantic regression file that exercises tool-level duration input → m/s transport → selected display echo for metric/imperial run, 100m/100y swim, and 500m row; explicitly prove 3.5714285 m/s renders as approximately 280 s/km and a 280 s/km write returns the same m/s; assert [77.5,100] is copied untouched into a pace-zone definition. Extend response converter tests only where necessary and run response, athleteprofile, tools, and intervals targets.
