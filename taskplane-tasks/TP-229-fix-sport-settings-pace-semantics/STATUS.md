...
- [ ] R017: Complete the repository-wide duration-shaped fixture audit
...
- Step 4 plan: change the checked-in athlete profile fixture to a RUN setting with `threshold_pace: 3.5714285`, display-only `MINS_KM`, `pace_load_type: RUN`, and percentage zones `[77.5,90,100]`; strengthen its real client decode test to assert type, m/s, presentation/load metadata, percentages, and names. Replace all remaining duration-shaped test records in get-athlete-profile readiness and data-quality tests with `ThresholdPace` m/s, explicit applicable display units, and ascending percentage zones; do not retain `PaceThreshold` duration fixtures. Add a table-driven semantic regression file that exercises tool-level duration input → m/s transport → returned upstream m/s selected-display echo for metric/imperial run, 100m/100y swim, and 500m row; explicitly prove 3.5714285 m/s renders as approximately 280 s/km and a 280 s/km write returns the same m/s; assert [77.5,100] is copied untouched into a pace-zone definition. Extend response converter tests only where necessary and run response, athleteprofile, intervals, get-athlete-profile, data-quality, and semantic tool targets.
