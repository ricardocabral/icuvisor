...
| Terrain execution analysis for runs | "Did I pace climbs, flats, and descents differently, and where did I fade?" | Activity streams for distance/time plus elevation or grade (`altitude`, `grade_smooth`) and enough samples for segment stats. | Existing `get_climb_segments` detects bounded sustained positive-grade climbs only; a separate follow-up terrain analyzer would group climbs, flats, and descents and reuse `compute_activity_segment_stats` formulas. | TP-253's narrow detector does not auto-classify broader terrain execution, compare pacing, or identify fade; grade/elevation streams may be missing or noisy. | Narrow climb detection `existing` via TP-253; broader terrain execution remains `new follow-up` candidate |
...
| Commute-aware training-day filtering | "Can you separate commute rides from training analysis?" | The upstream activity `commute` flag, plus explicit activity windows and analyzer source coverage. | Future analyzer/reporting follow-up may offer caller-selected commute inclusion/exclusion; current activity reads expose the flag but analyzers do not silently exclude commute activities or infer the flag from names/tags. | No automatic plan-health, training-day, or load reclassification is added by the activity metadata read path. | `new follow-up` candidate; TP-254 exposes source field only |
| Explicit repeated-workout progression evidence | "What factual evidence changed across these supplied repeated sessions?" | Ordered activity IDs plus details, intervals, optional extended metrics, linked prescriptions, subjective fields, and optional wellness rows. | Existing full-toolset `compute_workout_progression` reports deterministic rows/deltas and source gaps. | No name-based matching, progression/readiness score, physiology inference, autonomous coaching recommendation, or calendar write; deferred matching and physiology fields remain outside this surface. | `existing` via TP-258 |
