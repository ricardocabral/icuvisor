...
- [x] `get_extended_metrics` field set per PRD §7.2.C — running dynamics, DFA α1, W' balance, cardiac decoupling, HR drift, aerobic decoupling, zone distributions, IF/VI/polarization, TRIMP, strain score and its critical-power/W'/P-max model parameters (`strain_score_cp_watts`, `strain_score_w_prime_kj`, `strain_score_p_max_watts`), HR/pace/power load, L/R balance, RPE/feel/session-RPE, compliance %, device name — gated by upstream availability (PRD §7.4 #4).
...
- [ ] Nutrition macros + calories-label clarification in wellness and activity reads ([#22](https://github.com/ricardocabral/icuvisor/issues/22)). Two-part scope. (a) Wellness already surfaces `carbohydrates` / `protein` / `fatTotal` / `kcalConsumed`, but under raw upstream camelCase keys with no unit suffix and no `_meta` unit label — rename to disambiguated, unit-suffixed keys (`carbohydrates_g`, `protein_g`, `fat_g`, `kcal_consumed`) and label them the way subjective scales already are. (b) Activity reads (`get_activities` / `get_activity_details`, shared `activityRow`) surface only `calories_burned` — add intervals.icu's native per-activity nutrition, disambiguating the three carb concepts that must never collapse to a bare `carbs` key: carbs ingested during the activity, carbs used/burned (an upstream estimate), and carb intake rate in g/h. Prefer reading intervals.icu's own value over computing one server-side — a server-computed carbs/hour drifts from upstream's (gesteves/domestique#61 learned this and reverted to the upstream field); if a rate must be derived, flag it `_meta`-derived with a documented formula. Confirm `calories_burned` (activity, active burn) stays distinct from wellness `kcal_consumed` (intake). All field names verified against intervals.icu's public API per the clean-room rule.
