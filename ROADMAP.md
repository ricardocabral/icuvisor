...
- [x] `workout_doc` write-path serializer: structured steps round-trip back to the description-string DSL on upload (intervals.icu rejects structured `workout_doc` on writes — mvilanova #56). Read → modify → write → read fidelity locked by golden-file tests.
