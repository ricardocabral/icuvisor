...
func TestCreateWorkoutWithFreeTextOnlyPreservesDescription(t *testing.T) {
	...
	result, err := tool.Handler(context.Background(), Request{Name: tool.Name, Arguments: json.RawMessage(`{"name":"Free Text","sport":"Run","description":"  Coach note\nKeep this verbatim.  ","tags":["coach"]}`)})
	if err != nil {
		t.Fatalf("Handler() error = %v", err)
	}
	if len(client.calls) != 1 {
...
func TestCreateWorkoutGoldenFixtureRoundTripFromWorkoutDocSerializer(t *testing.T) {
	...
	_, err = tool.Handler(context.Background(), Request{Name: tool.Name, Arguments: json.RawMessage(`{"name":"Golden","sport":"Ride","workout_doc":` + string(rawDoc) + `}`)})
	if err != nil {
		t.Fatalf("Handler() error = %v", err)
	}
	if len(client.calls) != 1 || client.calls[0].Description == nil || *client.calls[0].Description != wantDSL {
		t.Fatalf("description call = %#v, want golden DSL %q", client.calls, wantDSL)
	}
}
...
func TestCreateWorkoutRejectsBadArguments(t *testing.T) {
	...
	for _, raw := range []string{
		`{"sport":"Ride"}`,
		`{"name":"Tempo"}`,
		`{"name":"Tempo","sport":"Ride","description":"note","workout_doc":{"steps":[{"duration":600}]}}`,
		`{"name":"Tempo","sport":"Ride","unknown":true}`,
	} {
...
func TestCreateWorkoutRegistrationMetadata(t *testing.T) {
	...
	tool := findTool(t, registrar.tools, createWorkoutName)
