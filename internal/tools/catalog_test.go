...
func TestCatalogIncludesAnalyzerFamilyPlacement(t *testing.T) {
	...
	cases := map[string]string{
		...
		computeZoneEnergyName:           "Use when the prompt asks for timestamp-weighted mechanical work or kJ by configured power zone across activities in a bounded date range",
...
func TestCatalogSummariesUseFirstDescriptionSentence(t *testing.T) {
	...
	cases := []struct {
		...
		{name: getActivitiesName, want: "Scan an athlete-local date window and return a paginated activity index: terse unit-disambiguated summary rows with IDs, source/device hints, tags, explicitly requested custom_fields, and Strava-unavailable markers."},
