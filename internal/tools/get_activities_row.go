...
func shapeGetActivitiesResponse(activities []intervals.Activity, gearResolutions map[string]activityGearResolution, args GetActivitiesRequest, nextToken string, version string, timezoneFallback string, debugMetadata bool, unitSystem response.UnitSystem, customFieldCodes []string, asOfMeta *response.AsOfMetadata, shaping ...responseShaping) (any, error) {
	...
	meta := getActivitiesMeta{PageSize: args.PageSize, NextPageToken: nextToken, MoreAvailable: nextToken != "", IncludeFull: args.IncludeFull, Timezone: timezoneFallback, FieldSemantics: activityFieldSemantics(rows), DataAvailability: activityAvailabilityDiagnostics(rows, customFieldCodes), CustomFieldProvenance: activityCustomFieldProvenance(customFieldCodes)}
	if asOfMeta != nil {
