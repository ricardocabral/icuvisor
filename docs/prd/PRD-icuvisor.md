...
- `get_athlete_profile` — FTP, zones, sport settings, thresholds. Power threshold fields are distinct from zone-boundary arrays: `ftp_watts` is the upstream sport FTP threshold, `indoor_ftp_watts` is present only when intervals.icu exposes an `indoor_ftp` override, and `power_zones_watts`/HR/pace zone arrays are boundary values rather than alternate FTPs. Response metadata states this so assistants do not infer a separate indoor FTP from a zone boundary or planned-event Indoor flag.
