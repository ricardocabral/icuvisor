...
- `get_athlete_profile` — FTP, zones, sport settings, thresholds. Power threshold fields are distinct from zone-boundary arrays: `ftp_watts` is the upstream sport FTP threshold, `indoor_ftp_watts` is present only when intervals.icu exposes an `indoor_ftp` override, and `power_zones_watts`/HR/pace zone arrays are boundary values rather than alternate FTPs. Upstream `threshold_pace` is stored in meters per second; the response derives an explicit seconds-per-distance field solely from display-only `pace_units`, while `pace_zones_percent_of_threshold` preserves upstream percentage boundaries unchanged. Response metadata states these conventions so assistants do not infer a separate indoor FTP from a zone boundary or planned-event Indoor flag.
- `update_sport_settings` — write outdoor FTP, indoor FTP in watts, threshold HR, threshold pace, and per-sport zone definitions back to the athlete profile (forum #35 — assistants offered FTP updates that didn't land because there was no write path). Indoor FTP is a sparse threshold-only write, independent of the outdoor FTP value; no indoor-versus-outdoor ordering rule is inferred. Threshold-pace inputs use explicit athlete-facing duration units but are transported as upstream m/s; `pace_units` is display metadata and `pace_load_type` preserves or disambiguates Run/Swim handling. Pace-zone boundaries are percent-of-threshold values, never durations. Per-sport scoped (`sport: "Ride" | "Run" | …`). Gated by `ICUVISOR_DELETE_MODE` like other destructive writes when used to overwrite zone definitions, since a bad zone overwrite silently miscolours every historical activity.
- `create_sport_settings` — full-toolset-only, threshold-only creation for a sport that does not yet have settings. Requires `sport` plus at least one positive outdoor FTP, indoor FTP, threshold HR, or threshold pace; threshold-pace duration input is transported as upstream m/s using explicit Run/Swim display metadata. It cannot replace zones, request HR-zone recalculation, or apply settings to historical activities. If settings already exist for the sport, use `update_sport_settings` instead.
- `list_athletes`, `select_athlete` — coach mode.
- `get_fitness` — CTL/ATL/TSB trends, taper projections. `get_fitness_projection` can accept caller-supplied weekly training-plan load targets, including bridge rows from `get_annual_training_plan`, and deterministic daily overrides so projections do not depend on ad-hoc chat math.
- `get_today` — one-call athlete-local daily digest for "how's today looking?": today's CTL/ATL/TSB, wellness, completed activities, planned events, and NOTE/race annotations, terse by default with `include_full` raw section detail. Planned workout rows and completed activity rows expose explicit `workout_status` values/caveats so today's planned workout is not treated as completed unless linked by upstream data. Planned-event `indoor` is the upstream venue flag and is not a sport type or FTP selector.
...
- `get_activities` — date-range list; supports `include_unnamed` (issue #67), pagination, and inline `gear_id` / `gear_name` when upstream exposes gear IDs. Unresolved gear IDs carry an explicit `gear_resolution` status rather than a guessed name. Rows also expose source-labelled upstream `commute`, athlete-reported `feel` on the 1–5 scale, and native `rpe` from `icu_rpe` on the separate 1–10 exertion scale when present; absent values are omitted, commute is never inferred, and feel is never converted to RPE.
- `get_activity_details` — single-activity metadata, zones, metrics, and the same inline gear fields/resolution status as activity list rows, including source-labelled commute, 1–5 feel, and native 1–10 RPE when upstream provides them.
- `update_activity` — sparse, non-destructive completed-activity metadata write for `name`, free-text `description`, and set-only `carbs_ingested_g`. Athlete-logged carbohydrate intake accepts whole grams from 0 through 2147483647; zero is a logged zero, omission leaves the value unchanged, and clearing is not supported. `carbs_used_g`, commute, feel, and native `icu_rpe` remain read-only; the unverified upstream RPE write contract is not advertised. intervals.icu does not update Strava activities through this endpoint.
...
- `get_events`, `get_event_by_id` — calendar entries. Planned workout rows include explicit `workout_status` values (`planned`, `future`, `completed_linked`, `missed_or_skipped`) with caveats when upstream data cannot distinguish skipped from missed or expose deleted/cancelled rows. The row-level `indoor` boolean is the upstream planned-event venue flag; it is separate from `type`, `tags`, FTP thresholds, and zone data, and response metadata tells assistants to use `sport_settings[].indoor_ftp_watts` only when present rather than hallucinating indoor/outdoor FTP splits. Rows also include a compact `workout_doc_summary`; when athlete sport settings are already available and a structured target can be resolved safely, `workout_doc_summary.target_previews[]` adds `{step, path, description, family, target, preview, basis, repeat_reps?}` entries such as `88-94% FTP` → `220-235 W`. Unsupported, ambiguous, or threshold-missing targets omit `target_previews` rather than emitting null placeholders.
- `add_or_update_event` — structured workout, race, or note. Returns a **terse** confirmation by default (issue #89). Preserves intervals.icu's distinction between `description` (free text — athlete/coach notes, pacing, nutrition, race countdown) and `workout_doc` (structured steps). On edit, `description` is written through **verbatim** unless the caller explicitly opts into structured normalization; `workout_doc` is the only field that accepts structured-block syntax. Silent normalization of free text is treated as a destructive operation and must not happen by default. Creates preflight same-day calendar events, skips exact duplicates whose writable create shape already exists, returns same-day conflict metadata for non-identical events, and includes `_meta.confirmation_status` so callers can distinguish an upstream returned write payload from a preflight duplicate skip; upstream does not expose an atomic compare-and-set idempotency key, so near-concurrent creates can still race. Accepts a `tags` array (e.g. `["sweet-spot", "indoor"]`) which round-trips through reads, and a top-level `indoor` boolean for intervals.icu's planned-event Indoor toggle (e.g. indoor trainer rides, commonly alongside `type: "VirtualRide"`). **Upload-asymmetry note**: intervals.icu emits structured `workout_doc` JSON on reads (an object with `steps[]`, where each step has fields like `duration`, `power` / `hr` / `pace`, optional `reps` for repeat blocks, optional nested `steps[]` for the body of a repeat), but the **write endpoints silently ignore `workout_doc`** — the structured steps must be re-encoded into intervals.icu's plain-text workout DSL and submitted in the `description` field. icuvisor owns this serialization so callers pass structured steps and never see the asymmetry. Implementation requirements:
  - **Tool input** accepts a structured `steps[]` array and a free-text `description` independently, and **either or both may be set in the same call**. The LLM must never be forced to choose between adding structure and attaching coach/athlete notes, and must never be nudged to fall back to a separate `NOTE` event just to preserve prose. This mirrors how intervals.icu's own description field works — DSL step lines and free-text headers/comments/notes coexist in a single field.
  - **On write**: the server composes the outgoing DSL `description` from whichever inputs are present.
    - If only `description` is provided: submit verbatim, zero normalization.
    - If only `steps[]` is provided: serialize to DSL and submit; the resulting string contains the structured steps and nothing else.
    - If both are provided: serialize `steps[]` to DSL and **interleave** the free-text `description` prose verbatim around the structured steps, preserving the caller-specified placement (top-of-description notes, between-block headers, trailing nutrition/pacing notes). Silent reformatting, reordering, or paraphrasing of athlete/coach prose is a destructive op and is not allowed.
    - In all cases the upstream `workout_doc` field is not sent — the DSL string in `description` is the only write channel intervals.icu accepts.
  - **DSL serializer** must handle: warmup/cooldown blocks, steady-state intervals (`duration` + single target), repeats (`Nx { ... }`), ramps (start→end target), free-form rest, and target types: % FTP, watts, % threshold HR, bpm, % threshold pace, pace (`mm:ss/km` or `mm:ss/mi` per athlete `preferred_units`), zone references (`Z1`–`Z7`), and RPE. Percent targets are expressed as percent points (`95` for 95% FTP/HR/pace), not fractional ratios (`0.95`); fractional percent-like targets must be rejected or diagnosed rather than serialized as `0.95%`. Durations in seconds, minutes, or hours; distance steps serialize canonical `mtr`, `km`, `mi`, or `yrd` suffixes, with `yrd` for yard-based pool-swim distances (aliases `yd`, `yard`, `yards` are accepted on input and canonicalize to `yrd`). A structured step's `description` is label/comment text only: duration and distance tokens such as `2h15m`, `45m`, `400mtr`, or `5km` must be rejected there so each step has exactly one duration/distance source. The canonical encoding is locked behind golden-file tests so subtle reformatting drift (whitespace, repeat syntax, target ordering) cannot regress silently.
  ...
  - **Structured-portion validation** is provided by a dedicated read-only `validate_workout` tool (listed below) that mirrors intervals.icu's own parser: it extracts and validates the structured-step lines, leaves prose / headers / comments untouched, and returns structured diagnostics keyed to the step it could not parse along with the canonical DSL the server would have submitted. `validate_workout` accepts either a raw `description` string, a structured `steps[]` array, or both — same coexistence rule as the write tools above. The same validator runs as a `dry_run: true` mode on `add_or_update_event`, `create_workout`, and `update_workout` so the LLM can preview the canonical merged DSL and any diagnostics without an upload. Validation must never be a precondition for writes (an LLM that knows the DSL can write directly), and must never reject prose it cannot parse — only structured-step lines are subject to validation.
- `validate_workout` — read-only. Parses and validates the structured-step portion of a workout (either a DSL `description` string, a structured `steps[]` array, or both), and returns the canonical merged DSL plus diagnostics (errors, warnings, lossy fields, common-mistake hints such as `'m' is minutes; use 'mtr' for meters`). Mirrors intervals.icu's parser: prose / headers / comments are passed through untouched, only structured-step lines are validated. Used by the LLM as a pre-flight check before write tools, and shared as the implementation of `dry_run: true` on `add_or_update_event` / `create_workout` / `update_workout`.
- `add_unavailable_date_range` — create Sick, Injured, or Holiday/time-off calendar blocks over an inclusive athlete-local date range. The tool writes one non-destructive per-day `Unavailable` event, uses deterministic external IDs for retry-safe repeats, skips matching duplicates, reports same-day conflicts such as workouts without deleting or overwriting them, rejects unsupported categories/aliases, and keeps raw payloads behind `include_full: true`.
- `delete_event`, `delete_events_by_date_range` — destructive. **Gated by `ICUVISOR_DELETE_MODE` env var, not a tool argument** (see §7.2.D — model-controlled `confirm: true` flags are not a credible safety guard). `delete_event` echoes the sparse pre-delete event payload and marks `_meta.confirmation_status` as post-delete-unverified because the tool does not perform a second destructive visibility probe after the upstream delete call succeeds.
- `get_training_plan` — fetch plan (forum #70).
- `get_annual_training_plan` — read-only ATP/periodization summary from existing PLAN, TARGET, and NOTE calendar events. NOTE rows with a trimmed non-empty upstream `plan_applied` timestamp are returned as `notes` with `status: "atp_generated"`; ordinary personal NOTE rows with null/empty provenance remain available separately as `context_notes` with `status: "personal_context"` and never contribute to ATP note counts, IDs, or recovery conclusions. Names, tags, `for_week`, and localized note text are not provenance or recovery signals. The response also returns phases, weekly load/time/distance targets, caveats for missing athlete-level parameters, and `_meta.projection_bridge.weekly_plan_targets` rows derived only from explicit TARGET loads for use with `get_fitness_projection.weekly_plan_targets`.
- `propose_annual_training_plan` — read-only deterministic season-plan proposal from caller-provided goal date, load/hour targets, caps, sports, strength context, optional taper scenario inputs, and optional explicit `sport_allocations`. Each allocation supplies a normalized named discipline, `load_share_pct` values totaling 100 within the documented numeric tolerance, and a `weekly_session_count`; the response adds deterministic `weekly_targets[].sport_targets[]` rows for Ride, Run, Swim, or other caller-named disciplines, reconciling allocated load/hours to every rounded parent row. `sports` remains context-only when allocations are omitted. `taper_weeks` accepts 1–3 final contiguous weeks (default 1, clamped to a shorter horizon) and `taper_target_load_pct` accepts 1–100% of `target_weekly_load` (default 60%). The response exposes requested/resolved values, input/default sources, assumptions, and any horizon clamp in `_meta.taper_scenario`; taper and sport allocation values are rounded deterministically, while the projection bridge remains aggregate parent loads. These are transparent caller-selected load/session targets, not availability, priority, intensity, suitability, physiology, or medical recommendations, and never scheduled workouts. Returns proposed phases, weekly load/time targets, optional discipline targets, recovery weeks, race anchors, assumptions, warnings, and projection-ready weekly targets without writing calendar data.
- `apply_annual_training_plan` — full-toolset write path for `propose_annual_training_plan` outputs. Defaults to dry-run, requires a returned preview token before commit, writes only deterministic Icuvisor-owned ATP phase notes with retry-safe external IDs, and reports protected existing ATP rows, races, notes, workouts, unavailable rows, and unknown categories instead of deleting or overwriting them.
- `apply_training_plan` — instantiate a workout-library folder onto the calendar from a start date. Dry-runs report existing same-day events and duplicate plan dates before writes, including conflict category/type/name/date details for protected non-workout calendar items. Non-dry runs skip exact duplicate existing plan events and duplicate same-day planned entries instead of creating another copy; `replace_existing` only replaces workout conflicts and reports/skips protected notes, races, unavailable blocks, holidays, sick/injured blocks, and unknown non-workout rows rather than deleting them.
- _Strength training data_ — included only if the intervals.icu API exposes first-class endpoints and schemas; until then icuvisor supports best-effort gym calendar notes/simple supported events, with implementation criteria tracked in [`docs/upstream-gaps/strength-training.md`](../upstream-gaps/strength-training.md) (forum #70).
...
- **No silent imputation.** Missing days are skipped and counted by analyzers whose documented method permits sparse samples: `_meta.missing_days: 3`, `_meta.missing_action: "skip"`. Never forward-fill without an explicit caller opt-in. `compute_training_monotony` is stricter: missing or otherwise defective daily coverage makes its statistic unavailable with `_meta.missing_action: "refuse"`.
- **Wellness freshness is response-visible.** HRV/HRV-SDNN analyzers surface stale or absent current-window wellness data with `freshness_status`, `caveats`, and `_meta.assumptions.wellness_freshness` (latest sample date, window end date, missing-day count, and stale provider rows when known) so assistants do not treat old bridge data as fresh HRV suppression evidence.
...
- `analyze_trend` — rolling mean, slope, Δ%, week-over-week change for a metric over a window vs a baseline window. Use when the user asks whether something is improving, worsening, or flat (CTL ramp, weekly TSS, pace-at-LT2, weight, HRV). HRV/HRV-SDNN trends include visible freshness caveats when the latest sample predates the trend window end or bridge provenance is stale.
...
- `compute_training_monotony` — full-toolset, read-only Foster-style mean daily `training_load` divided by population SD across an inclusive 1–31-day athlete-local window. It fetches `get_training_summary`, accepts only one raw finite non-negative load number per expected date (including explicit zero), and refuses missing, duplicate, malformed, invalid, out-of-window, negative, or non-object coverage rather than imputing or skipping it. Zero variance is undefined; the descriptive statistic is never a readiness, risk, medical, adaptation, or sport-variety score.
...
- `get_climb_segments` — full-toolset, read-only sustained-climb detector for prompts asking which climbs occurred in one activity or comparing its climb segments. It fetches canonical distance/altitude and optional time/heart-rate/power streams, applies bounded grade, gain, gap, and bridged-loss parameters, and returns concise segment metrics plus explicit missing/noisy-data diagnostics instead of raw arrays. It is a deterministic data summary only: it does not auto-classify broader terrain execution, make pacing/coaching/physiology claims, or infer unsupported values.
...
- `compute_workout_progression` — full-toolset, read-only evidence report for an explicitly ordered sequence of repeated completed activity IDs. It compares prescribed/completed structure, target-family adherence, stability, duration/recovery deltas, source-labelled feel/RPE, and optional wellness context; mixed sports, mismatched structure, and missing sources remain explicit `not_comparable` or `insufficient_evidence` states. It never infers equivalence from names, assigns a progression/readiness score, recommends intensity/recovery/repetition changes, or writes calendar data.
...
The generated tool catalog is the source of truth for the current registered tool count. As of the current generated website data, the catalog has 73 tools total: 30 in `core` and 43 additional `full` tools behind `ICUVISOR_TOOLSET=full`.
...
- **Athlete ID normalization** — accept both intervals.icu ID shapes: the `i12345` form (accounts created on intervals.icu) and the bare-numeric form (Strava-linked accounts, issue #40). The leading `i` is part of the ID, not a display convenience — a bare-numeric ID is never rewritten with one. Normalization trims whitespace, lowercases the optional `i`, and validates digits; it does not add or strip the prefix.
- **Strava-imported activity handling** — intervals.icu blocks Strava-synced activities from its public API per Strava's ToS. Tools must detect the blocked state and return a structured `unavailable` marker with the stable reason `strava_blocked` and an actionable workaround: `Open the intervals.icu Connections page for the activity's original device provider and click Download old data so historical activities are re-imported directly from that provider instead of through Strava's restricted API.` When explicit payload evidence identifies the native provider, the workaround names it, for example: `Open the intervals.icu Connections page, choose Wahoo, and click Download old data so historical activities are re-imported directly from Wahoo instead of through Strava's restricted API.`
...
The full generated tool surface costs meaningful tokens to load into a conversation — every conversation, every model, every client load. icuvisor defaults to exposing a curated **`core`** subset covering the daily-use path (read activities, fitness, wellness, events; common writes; and benchmark-proven analyzers). Power users and coaches opt in to the **`full`** surface via `ICUVISOR_TOOLSET=full` in their MCP client config. As of the current generated catalog, `core` exposes 30 tools and `full` adds 36 more.

A small `icuvisor_list_advanced_capabilities` tool lives in `compact` and `core` so the LLM can discover what's hidden and tell the user how to enable `core` or `full` when a prompt requires an advanced tool. This addresses the "tool selection accuracy" failure mode that grows with surface size — smaller models pick the wrong tool less often when the catalog is smaller.
...
It also ships thirteen curated MCP Prompts (`training_analysis`, `ride_analysis`, `fueling_review`, `recovery_check`, `weekly_planning`, `weekly_review`, `coaching_handoff`, `shareable_training_report`, `plan_health_review`, `masters_plan_review`, `race_week_taper`, `coach_roster_triage`, `coach_athlete_onboarding`) so users on clients that surface prompts get a "what can this thing do?" entrypoint without having to learn the tool catalog. `fueling_review` is a read-only, source-labelled review of logged activity intake and daily nutrition fields: it calculates logged grams/hour only from non-negative `carbs_ingested_g` and positive `moving_time_seconds`, reports coverage and missing logs, never substitutes `carbs_used_g` or daily wellness macros, and does not prescribe nutrition targets. `coaching_handoff` is a read-only, privacy-safe workflow for manually carrying reviewed user decisions and source-labelled, athlete-local evidence into a fresh client conversation without server-side conversation storage. `masters_plan_review` is a strictly read-only, evidence-limited audit of an existing plan: it separates sourced evidence from athlete-stated preferences, suppresses only conclusions unsupported by missing or stale data, and never derives an age policy, medical conclusion, opaque score, or automatic calendar change.
...
3. **The intervals.icu API supports strength training and training plan retrieval.** Strength-training tools require endpoint, schema, terse/full response, safe-write, and round-trip evidence before entering the catalog; best-effort gym notes/events do not imply first-class strength-set support. _(Validate during tool-catalog implementation; track the strength criteria in [`docs/upstream-gaps/strength-training.md`](../upstream-gaps/strength-training.md).)_
...
12. **A curated `core` toolset covers ≥90% of real prompts** based on curated public behavior patterns plus KR5 analyzer benchmark evidence. The current generated catalog exposes 30 `core` tools; the remaining 40 `full` tools (bulk ops, gear, sport settings, curves, histograms, custom items, workout-library writes, and specialized analyzers) are placed behind `ICUVISOR_TOOLSET=full`. _(Validate via opt-in telemetry on tool-call distribution after v0.5 dogfooding.)_
...
17. **Pace-unit enum coverage is exhaustive.** Upstream reports showed running/swim activities failed to deserialize due to missing units. The intervals.icu unit enum (as observed via black-box probing) appears on activity stream metadata, interval target definitions, and best-effort responses. Known members the implementation must handle without crashing:
    ...
    - Pace: `MINS_KM` (min/km), `MINS_MILE` (min/mi), `SECS_100M` (sec/100m — metric swim), `SECS_100Y` (sec/100yd — yard-pool swim), `SECS_500M` (sec/500m — row).
