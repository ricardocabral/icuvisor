...
- `get_annual_training_plan` — read-only ATP/periodization summary from existing PLAN, TARGET, and NOTE calendar events. Returns phases, weekly load/time/distance targets, recovery/context notes, caveats for missing athlete-level parameters, and `_meta.projection_bridge.weekly_plan_targets` rows that can be copied into `get_fitness_projection.weekly_plan_targets`.
