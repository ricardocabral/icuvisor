...
- `propose_annual_training_plan` — read-only deterministic season-plan proposal from caller-provided goal date, load/hour targets, caps, sports, strength context, and optional taper scenario inputs. `taper_weeks` accepts 1–3 final contiguous weeks (default 1, clamped to a shorter horizon) and `taper_target_load_pct` accepts 1–100% of `target_weekly_load` (default 60%). The response exposes requested/resolved values, input/default sources, assumptions, and any horizon clamp in `_meta.taper_scenario`; taper load is rounded once and copied to the projection bridge. These are transparent caller-selected load scenarios, not intensity, suitability, physiology, or medical recommendations. Returns proposed phases, weekly load/time targets, recovery weeks, race anchors, assumptions, warnings, and projection-ready weekly targets without writing calendar data.
