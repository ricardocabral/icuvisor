...
| `GET /api/v1/athlete/{id}/events{format}` | `icu_training_load`, `load_target`, `time_target`, `distance_target`, `target`, `icu_intensity`, plan linkage fields | Per-event values only, not athlete-level periodization settings. |
