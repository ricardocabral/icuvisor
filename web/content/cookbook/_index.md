...
If your client supports [MCP prompts]({{< relref "/reference/resources-prompts" >}}), icuvisor ships seven curated ones (`training_analysis`, `recovery_check`, `weekly_planning`, `weekly_review`, `plan_health_review`, `race_week_taper`, `coach_roster_triage`) you can invoke directly — they carry the same guardrails server-side. The cookbook recipes are the portable equivalent: plain text that works in any client, including ones with no prompt support. Each recipe notes its matching MCP prompt where one exists.
