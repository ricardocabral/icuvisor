...
| `athlete_id`     | string | required unless coach mode supplies `coach.default_athlete_id` | intervals.icu athlete ID. Surrounding whitespace is trimmed, an optional leading `I` is lowercased to `i`, and the remaining characters must be digits. Both ID shapes are preserved: `12345` remains `12345`, and `i12345` remains `i12345`. |
